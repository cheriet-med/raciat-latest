...
interface Product {
  ...
  owner_title: any;
  category: string;
  existing_image?: string; // For displaying existing image
}
...
export default function BlogPostForm() {
  ...
  // Fetch product data when component mounts or query changes
  useEffect(() => {
    if (query) {
      fetchProductData();
    }
  ...
  const fetchProductData = async () => {
    ...
    setIsLoading(true);
    try {
      const productResponse = await fetch(`${process.env.NEXT_PUBLIC_URL}postid/${query}`, {
        method: 'GET',
        headers: {
      ...
      setErrorMessage('فشل في تحميل بيانات المقال');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `فشل في ${query ? 'تحديث' : 'إنشاء'} المقال`);
      }
      ...
      setErrorMessage(
        error instanceof Error ? error.message : `حدث خطأ أثناء ${query ? 'تحديث' : 'إنشاء'} المقال. يرجى المحاولة مرة أخرى.`
      );
    } finally {
        ...
        <div className="px-4">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="p-8 text-center">
              ...
              <p className="text-xl mt-4 text-gray-600">جاري تحميل بيانات المقال...</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-sec px-8 py-6">
            <h1 className="text-3xl font-bold text-white">
              {query ? 'تعديل المقال' : 'إضافة مقال جديد'}
            </h1>
            <p className="text-white mt-2">
              {query 
                ? 'يمكنك تعديل المقال الحالي. يرجى ملء جميع الحقول المطلوبة المشار إليها بعلامة النجمة.'
                ...
                >
                  {isSubmitting ? (
                    <span className="flex items-center gap-2 justify-center">
                      ...
                      {query ? 'جاري تحديث المقال...' : 'جاري إنشاء المقال...'}
                    </span>
                  ) : (
                    query ? 'تحديث المقال' : 'إنشاء مقال'
                  )}
                </button>
