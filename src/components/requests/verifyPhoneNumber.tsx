...
const VerifyPhoneOTP = ({ 
  ...
  const handleVerifyOTP = async () => {
    ...
    const responses = await fetch(`${process.env.NEXT_PUBLIC_URL}infoid/${infoId}`, {
      ...
      setError(err instanceof Error ? err.message : "فشل التحقق من كلمة المرور لمرة واحدة (OTP). يُرجى التحقق من رمزك.");
    } finally {
  ...
  return (
    <div className="relative">
      {/* Verify Button */}
      <button 
        className="text-2xl font-medium text-sec hover:text-secondary flex items-center"  
                ...
                <div className="flex justify-center mt-4">
                  {countdown > 0 ? (
                    ...
                    <button
                      ...
                      disabled={isSending}
                    >
                      {isSending ? 'إرسال...' : 'إعادة إرسال OTP'}
                    </button>
                  )}
                </div>
