...
export default function EditeOrderForm() {
...
console.log(orderId)
  // Fetch order data when component mounts and session is ready
  useEffect(() => {
    // Only fetch when we have both orderId and session is loaded
    if (orderId && status === 'authenticated' && session?.user?.id) {
      fetchOrderData();
    }
  ...
  const fetchOrderData = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL}orderid/${orderId}`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`فشل في تحميل بيانات الطلب (${response.status})`);
      }
      ...
      setOrder({
        user: orderData.user || session?.user?.id,
        name: orderData.name || '',
        status: orderData.status || 'قيد المراجعة',
        created_at: orderData.created_at || new Date().toLocaleDateString('ar-EG'),
        image: null,
        ...
        hurry: orderData.hurry || '',
        surface: orderData.surface || '',
        latitude: orderData.latitude || '',
        longitude: orderData.longtitude || '',
        description: orderData.description || '',
        min_price: orderData.min_price || '',
        max_price: orderData.max_price || '',
        badrooms_number: orderData.badrooms_number || '',
      ...
      if (orderData.image) {
        setMainImagePreview(process.env.NEXT_PUBLIC_IMAGE+'/'+orderData.image);
      }
      ...
      setErrorMessage(
        error instanceof Error 
          ? error.message 
          : 'فشل في تحميل بيانات الطلب. يرجى التحقق من الاتصال والمحاولة مرة أخرى.'
      );
    } finally {
  ...
  const removeMainImage = () => {
    setOrder(prev => ({ ...prev, image: null, image_url: undefined }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      orderFormData.append('longitude', order.longitude);
      ...
      const orderResponse = await fetch(`${process.env.NEXT_PUBLIC_URL}orderid/${orderId}`, {
        method: 'PUT',
        headers: {
          "Authorization": `Token ${process.env.NEXT_PUBLIC_TOKEN}`,
        },
      ...
      if (!orderResponse.ok) {
        ...
        throw new Error(errorData.message || 'فشل في تعديل الطلب');
      }

      setSuccessMessage('تم تعديل الطلب بنجاح!');
      ...
      setErrorMessage(
        error instanceof Error ? error.message : 'حدث خطأ أثناء تعديل الطلب. يرجى المحاولة مرة أخرى.'
      );
    } finally {
        ...
        <div className="px-4">
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="p-8 text-center">
              <div className="w-12 h-12 border-4 border-sec border-t-transparent rounded-full animate-spin mx-auto"></div>
              <p className="text-xl mt-4 text-gray-600">جاري تحميل بيانات الطلب...</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="p-8 text-center">
              ...
              <button
                ...
                className="px-6 py-3 bg-sec text-white font-semibold rounded-xl hover:bg-prim transition-all"
              >
                إعادة المحاولة
              </button>
            </div>
      ...
      <div className="px-4">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-sec px-8 py-6">
            <h1 className="text-3xl font-bold text-white">تعديل الطلب #{orderId}</h1>
